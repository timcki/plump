...
**Display probe.** Settings > Display > Display Probe measures what the datasheet reading got wrong: each step runs a clean clear with the top half black and the bottom half white, writes the four {RED, BW} bit patterns as four portrait columns (left to right 00, 01, 10, 11) into both planes, then runs a custom LUT with a single live row (`Screen::display_probe`, `AppContext::request_display_probe`; Select steps through VSL / VSH1 / VSH2 in row 1 and VSL in row 2, Back ends it). The column that moves says which plane is the high bit of the LUT index; whether its black half lightened or its white half darkened says the pulse's polarity. The panel is held still between steps (`display_probe_hold` drops redraws) and the exit repaints with a clean clear. The serial log carries the legend and the step timing.
