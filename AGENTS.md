...
**AA page turns are deltas.** During a gray pass the strip draws the whole frame into both planes: the content (1 = white, as the panel will hold it after the revert) goes to RED RAM and, with the AA edge pixels cleared, to BW RAM. Plain pixels are therefore 1,1 or 0,0, no-change rows under either LUT bit order, and the single AA state (RED=1, BW=0) has the same lift waveform in both remaining rows of `LUT_GRAYSCALE` and the same revert in `LUT_GRAYSCALE_REVERT`; the order the controller reads the planes in was measured to be the opposite of the datasheet reading, so nothing depends on it. After the revert pass RED describes the panel exactly, `screen.rs` marks the planes synced, and the page turn is a single BW write plus a delta DU, the same path as a non-AA turn. Before this, plain pixels carried no content through the gray pass and every post-AA turn re-drove the whole panel through inv_red, a same-direction pulse over the entire background per turn that accumulated as a grey haze (worse on quick turns, where an abandoned frame re-drove twice). Every 1-bit drawing primitive writes the content plane in both modes, so an app must draw its complete frame in a gray pass, not only its AA glyphs.
