...
impl EpubState {
    ...
    // ── layout (PIDX v2) persistence ───────────────────────────────
    //
    // load / save / invalidate now live in `super::layout::cache`.
    // call sites in `paging.rs` and `mod.rs::on_resume` use the
    // PIDX v2 byte-layout records (`LayoutIdxHeader`,
    // `ChapterLayoutDir`, `PageRecord`, `LineRecord`) directly via
    // a `LayoutKey` cache key; legacy v1 (`PageIdxHeader` /
    // `ChapterPageDir`) is gone and any old PIDX bytes are rejected
    // by the loader and rebuilt on next preindex.
}
