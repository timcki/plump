...
pub fn save_cover_thumb(
    ...
    k.sd().write_in_plump_subdir(dir, COVER_THUMB_FILE, &header)?;
    k.sd().append_in_plump_subdir(dir, COVER_THUMB_FILE, &img.data)?;
    Ok(())
}
...
pub fn load_cover_thumb(k: &mut KernelHandle<'_>, dir: &str) -> Option<DecodedImage> {
    let size = k.sd().file_size_in_plump_subdir(dir, COVER_THUMB_FILE).ok()?;
    if size < 5 {
        return None;
    }
    let mut header = [0u8; 4];
    k.sd().read_chunk_in_plump_subdir(dir, COVER_THUMB_FILE, 0, &mut header)
        .ok()?;
    ...
    k.sd().read_chunk_in_plump_subdir(dir, COVER_THUMB_FILE, 4, &mut data)
        .ok()?;
    Some(DecodedImage {
        width,
        height,
...
pub fn has_cover_thumb(k: &mut KernelHandle<'_>, dir: &str) -> bool {
    k.sd().file_size_in_plump_subdir(dir, COVER_THUMB_FILE)
        .map(|s| s >= 5)
        .unwrap_or(false)
}
